...
#### 1.2 物理模块年级对齐与教学顺序优化 ✅ 已完成 (2026-02-16)

**重要发现**: 根据用户反馈，Basel地区物理课程通常从Sek 3开始。已完成全量历史高保真资料找回，并重新按教学规律排列。

**物理模块排列顺序 (教学逻辑：基础 -> 核心 -> 应用 -> 进阶)**:
1.  **SP3.01 // 测量与单位**: 物理基石，量化世界。
2.  **SP3.02 // 力与运动**: 经典力学核心。
3.  **SP3.03 // 能量与功**: 动力学进阶。
4.  **SP3.04 // 压强与流体**: 莱茵河应用场景。
5.  **SP3.05 // 简单机械**: 复杂力学应用（巴塞尔建筑工地）。
6.  **SP3.06 // 声学**: 波动物理初步。
7.  **SP3.08 // 几何光学**: 光路追踪与斯涅尔定律。

**模块映射与资源恢复清单 (Restoration Mapping)**:
| 巴塞尔新编号 | 内容来源 (从Git历史找回) | 核心特性 |
|:---:|:---|:---|
| **SP3.01** | 原 `sp1_01` (Measurement) | 包含诺华制药/瑞士手表高精度测量场景 |
| **SP3.03** | 原 `sp1_03` (Energy) | 包含莱茵河水电站/电车能量回收场景 |
| **SP3.04** | 原 `sp1_07` (Fluids) | 包含莱茵河潜水/货运浮力场景 |
| **SP3.05** | 原 `sp1_04` (Machines) | **完整背景版**：罗氏塔吊机、杠杆修复古建场景 |
| **SP3.08** | 原 `sp1_08` (Optics) | 斯涅尔定律精密实验台场景 |
| **GP1.xx** | 原 `gp5.x` 系列 | 现代物理：相对论、量子隧道、LHC对撞机 |
| **GP2.xx** | 原 `gp2.x` 系列 | 热力学核心系列 |

**状态更新**:
- [x] 完成物理模块 `en/physics.ts` 全量恢复 ✅
- [x] 完成物理模块 `cn/physics.ts` 全量恢复 ✅
- [x] 完成物理模块 `de/physics.ts` 全量恢复 ✅
- [x] 更新 `page.tsx` 导航顺序与教学规律一致 ✅
...
- [x] 审查SP2.01, SP3.02内容 ✅
- [x] 调整模块编号以符合Basel年级要求 ✅
- [x] 简化或重构SP2.02 (RLC电路) ✅
- [x] 更新物理模块文档 (CURRICULUM_PLAN_UPDATE_2026_v2.md) ✅
- [x] 三语言全量高保真翻译恢复 (Scenarios & Pros) ✅
...
- [x] 创建GB2.01 (Neurobiology) ✅
- [x] 创建GB3.02 (Immunology) ✅
- [x] 创建SC3.05 (Chemical Bonds Advanced) ✅
