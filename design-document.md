...
1.  **德语优先 (DE-First Core)**：
    *   鉴于项目首要受众为瑞士学生，德语 (DE) 作为**第一开发语言**。
    *   所有新模块的核心逻辑、题目池 (Quest Pool) 和教学提示，必须首先完成高质量的德语设计。

2.  **双语验证 (DE + EN Sync)**：
    *   为了验证 i18n 架构的灵活性，英语 (EN) 将同步进行开发。
    *   在编写代码时，文档和 UI 必须同时提供 DE 文本（首位）和 EN 文本（次位），作为一种“语义对齐检查”。

## 7. AI 协作与路由策略 (AI Routing Strategy)

为了实现最高效的开发，本项目采用“三位一体”的模型路由机制：

| 任务领域 (Domain) | 推荐模型 (Model) | 职责描述 (Responsibilities) |
| :--- | :--- | :--- |
| **系统操控 (System Control)** | **Gemini 3 (Native)** | 文件读写、终端命令执行、浏览器搜索、项目全局上下文管理。 |
| **后端逻辑 (Backend/Math)** | **GLM-4.7 (NVIDIA)** | 复杂数学公式推导（M-G09 微积分）、R3F 物理逻辑算法、Quest Pool 严谨性校验。 |
| **前端交互 (Frontend/i18n)** | **MiniMax-2.1 (NVIDIA)** | 德/英多语言文案润色、UI/UX 交互文本优化、教学提示 (Hints) 的语义对齐。 |

**执行原则**：
- 当 Gemini 遇到复杂的数学逻辑瓶颈时，必须主动通过 MCP 调用 `nvidia-glm` 进行咨询。
- 当涉及到 `i18n.ts` 的大规模更新时，优先调用 `nvidia-minimax` 获取最地道的德文表达。
- 所有的修改实施（Code Implementation）最终由 Gemini 完成，确保与 IDE 环境无缝集成。

3.  **中文延后 (CN-Lazy Architecture)**：
    *   中文 (CN) 将在架构层保留 Key 值定义，但内容暂时使用 DE/EN 占位。
    *   这种策略能显著减少每次对话的 Token 消耗，避免大规模重复相似的文案翻译。

4.  **文档编写规范**：
    *   项目内所有的 `.md` 任务文档（如阶段计划）应优先使用 **德文 + 英文** 混合编写，确保术语的国际化一致性。
