# 🏰 "Basilea 2026" Integrated Science Curriculum Roadmap
> **Strategic Alignment**: Basel-Stadt & Basel-Landschaft (Lehrplan 21 + Gymnasium)

This document outlines the consolidated development plan for the Science Theme Park, integrating the missing modules identified from the official Basel curriculum. It prioritizes the creation of "conceptual bridges" (e.g., Bonding, Cell Division) to ensure a seamless transition from Sekundarschule to Gymnasium.

---

## 🚦 Strategic Priorities: The "Bridging" Phase

We are currently in a critical phase of filling theoretical gaps to support advanced learning.

### 🎯 Immediate Priority (Phase 1 - The "Missing Bridges")
*Focus: Essential prerequisites for advanced Organic Chemistry and Genetics.*

| Priority | Code | Module Title | Subject | Rationale (Basel Curriculum Link) |
|:---:|:---:|:---|:---:|:---|
| **1** | **SC1.05** | **Bonding Bridge** | 🧪 Chem | **CRITICAL**. Prerequisite for Organic Chem (GC2.01). Explains *how* atoms connect (Ionic vs Covalent). |
| **2** | **SB1.03** | **Cell Division** | 🧬 Bio | **CRITICAL**. Prerequisite for Genetics (SB2.01). Mitosis/Meiosis mechanics are missing between Cell & Mendel. |
| **3** | **SP1.01** | **Measurement & Units** | ⚛️ Phys | **FOUNDATION**. SI Units, precision, and error analysis. The basis of all experimental science (Gymnasium goal). |
| **4** | **SC3.02** | **Organic Basics** | 🧪 Chem | **LOCAL CONTEXT**. Introduction to Carbon chemistry, essential for the Basel crypto-pharma theme. |

### 🗓️ Next Up (Phase 2 - Core Expansion & Visuals)
*Focus: completing the standard Sek I curriculum coverage.*

| Priority | Code | Module Title | Subject | Rationale |
|:---:|:---:|:---|:---:|:---|
| **5** | **SP1.08** | **Motion Master** | ⚛️ Phys | **GAP**. Kinematics (s-t, v-t graphs). Essential for understanding Newton's Laws deeply. |
| **6** | **SB1.02** | **Photosynthesis** | 🧬 Bio | **CORE**. Energy transformation in biological systems. Links to SC2.01 (Kinetics/Energy). |
| **7** | **SB2.02** | **Human Body** | 🧬 Bio | **CORE**. Systems biology (Digestion/Circulation). High student interest topic. |
| **8** | **SM1.07** | **Data Detective** | 📐 Math | **GAP**. Descriptive statistics (Boxplots, Histograms) as required by Lehrplan 21. |

---

## 📚 Full Curriculum Inventory (Target State)

### 📐 Mathematics (Basel Standard)
*Emphasis: Functions, Geometry, and Data Literacy.*

| Code | Module Title | Status | Notes |
|:---:|:---|:---:|:---|
| **SM1.01-06** | *Existing Core Modules* | ✅ Done | Geometry, Algebra, Integers, Equations, Ratio. |
| **SM1.07** | **Data Detective** | 📋 **NEW** | **Statistics:** Mean, Median, Boxplots, Histograms. |
| **SM2.01-08** | *Existing Intermediate* | ✅ Done | Binomials, Pythagoras, Linear Functions, Probability. |
| **SM2.09** | **Similarity Lab** | 📋 **NEW** | **Geometry:** Thales circle extension, similarity, intro to Trig ratios. |
| **SM3.01-03** | *Existing Advanced* | ✅ Done | Quadratics, Trig Circle, Exponentials. |
| **GM1-4** | *Gymnasium Modules* | ✅ Done | Calculus, Vectors, Complex Numbers. |

### ⚛️ Physics (Swiss KLT)
*Emphasis: From Phenomenology to Mathematical Description.*

| Code | Module Title | Status | Notes |
|:---:|:---|:---:|:---|
| **SP1.01** | **Measurement & Units** | 🚀 **NEXT** | **Foundations:** SI System, Prefixes, Accuracy. |
| **SP1.02** | Newton's Laws | ✅ Done | Forces & Motion. |
| **SP1.03** | Energy & Power | ✅ Done | Work & Conservation. |
| **SP1.05** | The Rhine Ferry | ✅ Done | Vectors & Relative Motion (Basel context). |
| **SP1.06** | Swiss Pendulum | ✅ Done | Harmonic Motion. |
| **SP1.07** | **Pressure & Buoyancy** | 📋 Planned | Hydraulics & Archimedes. |
| **SP1.08** | **Motion Master** | 📋 **NEW** | **Kinematics:** Reading and converting motion graphs. |
| **SP2.02** | Circuit Sandbox | ✅ Done | Electricity. |
| **GP1-4** | *Gymnasium Modules* | ✅ Done/Plan | particle physics, Quantum, Relativity. |
| **GP3.01** | **Thermodynamics** | 📋 **NEW** | **Heat:** Specific heat, phase changes, heat engines. |

### 🧪 Chemistry: The Pharma Hub
*Emphasis: Particle Model -> Bonding -> Reactions -> Organic.*

| Code | Module Title | Status | Notes |
|:---:|:---|:---:|:---|
| **SC1.01-04** | *Existing Intro* | ✅ Done | Mystery Lab, Moles, Atoms, Periodic Table. |
| **SC1.05** | **Bonding Bridge** | 🚀 **NEXT** | **Structure:** Ionic vs Covalent, Lewis Structures. |
| **SC2.01-04** | *Existing Physical Chem* | ✅ Done | Kinetics, pH, Gases, Solubility. |
| **SC3.02** | **Organic Basics** | 🚀 **NEXT** | **Organic:** Alkanes, Alkenes, Functional Groups. |
| **GC1-4** | *Gymnasium Modules* | ✅ Done/Plan | Redox, Advanced Organic, Equilibrium. |

### 🧬 Biology: The Life Science Nexus
*Emphasis: Systemic understanding from Cell to Ecosystem.*

| Code | Module Title | Status | Notes |
|:---:|:---|:---:|:---|
| **SB1.01** | Cell Factory | ✅ Done | Organelles & Structure. |
| **SB1.03** | **Cell Division** | 🚀 **NEXT** | **Process:** Mitosis & Meiosis (The missing link to Genetics). |
| **SB1.02** | **Photosynthesis** | 📋 Planned | Plant physiology & Energy. |
| **SB2.01** | Mendel's Garden | ✅ Done | Genetics & Heredity. |
| **SB2.02** | **Human Body** | 📋 Planned | Organ Systems. |
| **SB3.01** | **Ecosystem Dynamics** | 📋 **NEW** | **Ecology:** Food webs, Carbon cycle, Population dynamics. |
| **GB1-4** | *Gymnasium Modules* | Planned | Evolution, Neurobio, DNA, Biotech. |

---

## 🛠 Execution Protocol

1.  **Strict Phase 1 Compliance**: We will not move to Phase 2 until **SC1.05**, **SB1.03**, and **SP1.01** are "Skeleton" complete (functional logic).
2.  **Basel Context Check**: Every module must answer "How does this fit into a Basel student's life?" (e.g., Rhine swimming for fluid dynamics, Roche tower for statics/geometry).
3.  **Language**: All new modules must support `i18n` (DE/EN/CN) from Day 1 to support the local bilingual education environment.

*Plan updated: February 14, 2026*
