...
**⚠️ 重要教训：所有数学公式和物理单位必须使用 LaTeX 渲染，且必须遵循转义规范！**

#### 5.2.1 四反斜杠规则 (The Quadruple-Backslash Rule) - Sprint 5 标准
在 React 组件的属性、Hooks（如 `useQuestManager`）以及 i18n 模板字符串中，单反斜杠甚至双反斜杠往往在经过多次解析后会丢失或产生非预期的字符。
**强制标准**：在程序代码逻辑（如 `buildStagePool`）中定义的 LaTeX 字符串，必须使用 **四反斜杠 `\\\\`** 来表示一个 LaTeX 命令起始。

- ❌ **错误**: `"\text{kg}"` (JS 解析为 `ext{kg}`)
- ❌ **错误**: `"\\text{kg}"` (在某些模板替换或 i18n 场景下仍可能丢失一个 `\`)
- ✅ **正确**: `"\\\\text{kg}"`
- ✅ **环境换行 (LaTeX `\\`)**: 必须使用 **八反斜杠 `\\\\\\\\`** (确保最终传递给 KaTeX 的是原始的双反斜杠)。
  - 示例：`"\\\\begin{cases} x+y=1 \\\\\\\\ x-y=0 \\\\end{cases}"`

#### 5.2.2 禁止使用 Unicode 指数符号
严禁在 LaTeX 字符串中使用 Unicode 上标字符（如 ² , ³ , ⁴）。这会导致字体不统一、排版错位。统一使用 LaTeX 语法 `^n`。

- ❌ **错误**: `"m²"`, `"x² + y²"`
- ✅ **正确**: `"\\text{m}^2"`, `"x^2 + y^2"`

#### 5.2.3 物理单位格式化
所有单位必须使用 **正体 (roman font)**，包裹在 `\\text{}` 中。不应直接使用数学斜体。

- ❌ **错误**: `"9.8 m/s^2"` (渲染为斜体 $9.8 m/s^2$)
- ✅ **正确**: `"9.8 \\text{ m/s}^2"` (渲染为正体 $9.8 \text{ m/s}^2$)
