"use client";

import { useRef, useEffect } from "react";

interface TriangleCanvasProps {
  a: number;
  b: number;
  c: number;
  highlightRightAngle: boolean;
}

function TriangleCanvas({ a, b, c, highlightRightAngle }: TriangleCanvasProps) {
  const ref = useRef<HTMLCanvasElement | null>(null);

  useEffect(() => {
    const canvas = ref.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    // HiDPI Scaling
    const dpr = window.devicePixelRatio || 1;
    const size = 600;
    canvas.width = size * dpr;
    canvas.height = size * dpr;
    ctx.scale(dpr, dpr);

    const w = size;
    const h = size;
    ctx.clearRect(0, 0, w, h);

    const pad = 60;
    const maxLeg = Math.max(a, b);
    const scale = (Math.min(w, h) - pad * 2) / maxLeg;
    const ax = a * scale;
    const by = b * scale;

    const origin = { x: pad + 20, y: h - pad - 20 };
    const pA = origin;
    const pB = { x: origin.x + ax, y: origin.y };
    const pC = { x: origin.x, y: origin.y - by };

    // Fill with gradient
    const grad = ctx.createLinearGradient(pA.x, pA.y, pB.x, pC.y);
    grad.addColorStop(0, "rgba(57, 255, 20, 0.05)");
    grad.addColorStop(1, "rgba(0, 210, 255, 0.05)");
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.moveTo(pA.x, pA.y);
    ctx.lineTo(pB.x, pB.y);
    ctx.lineTo(pC.x, pC.y);
    ctx.closePath();
    ctx.fill();

    // Main Triangle Lines (Neon)
    ctx.save();
    ctx.strokeStyle = "#fff";
    ctx.lineWidth = 3;
    ctx.shadowColor = "rgba(255, 255, 255, 0.5)";
    ctx.shadowBlur = 15;
    ctx.beginPath();
    ctx.moveTo(pA.x, pA.y);
    ctx.lineTo(pB.x, pB.y);
    ctx.lineTo(pC.x, pC.y);
    ctx.closePath();
    ctx.stroke();
    ctx.restore();

    // Right Angle Marker
    if (highlightRightAngle) {
      ctx.save();
      ctx.strokeStyle = "#39ff14"; // Neon Green
      ctx.lineWidth = 3;
      ctx.shadowColor = "#39ff14";
      ctx.shadowBlur = 10;

      const s = 30; // marker size
      ctx.beginPath();
      ctx.moveTo(pA.x + s, pA.y);
      ctx.lineTo(pA.x + s, pA.y - s);
      ctx.lineTo(pA.x, pA.y - s);
      ctx.stroke();
      ctx.restore();
    }

    // Labels
    ctx.fillStyle = "rgba(255,255,255,0.9)";
    ctx.font = "bold 16px monospace";
    ctx.fillText(`a=${a}`, (pA.x + pB.x) / 2 - 20, pA.y + 30);
    ctx.fillText(`b=${b}`, pA.x - 50, (pA.y + pC.y) / 2 + 8);

    // Hypotenuse Label (rotated)
    ctx.save();
    const midX = (pB.x + pC.x) / 2;
    const midY = (pB.y + pC.y) / 2;
    // Calculate angle for rotation
    const angle = Math.atan2(pC.y - pB.y, pC.x - pB.x);
    ctx.translate(midX, midY);
    ctx.rotate(angle);
    ctx.fillText(`c=${c}`, -20, -15);
    ctx.restore();

  }, [a, b, c, highlightRightAngle]);

  return (
    <div className="relative w-full h-full bg-[#050505] rounded-xl border border-white/10 shadow-inner overflow-hidden">
      <canvas ref={ref} style={{ width: '100%', height: '100%' }} />
    </div>
  );
}

interface SpaceCanvasProps {
  a: number;
  b: number;
  c: number;
}

function SpaceCanvas({ a, b, c }: SpaceCanvasProps) {
  const ref = useRef<HTMLCanvasElement | null>(null);

  useEffect(() => {
    const canvas = ref.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    // HiDPI Scaling
    const dpr = window.devicePixelRatio || 1;
    const size = 600;
    canvas.width = size * dpr;
    canvas.height = size * dpr;
    ctx.scale(dpr, dpr);

    const w = size;
    const h = size;
    ctx.clearRect(0, 0, w, h);

    const scale = (Math.min(w, h) * 0.5) / Math.max(a, b, c);
    const cx = w / 2;
    const cy = h * 0.7;

    const iso = (lx: number, ly: number, lz: number) => {
      const ang = Math.PI / 6;
      const u = (lx - ly) * Math.cos(ang);
      const v = (lx + ly) * Math.sin(ang) - lz;
      return { x: cx + u * scale, y: cy + v * scale };
    };

    const O = iso(0, 0, 0);
    const A = iso(a, 0, 0);
    const B = iso(a, b, 0);
    const C = iso(a, b, c);
    const D = iso(0, b, 0);

    // Floor Grid (Dotted)
    ctx.strokeStyle = "rgba(255,255,255,0.1)";
    ctx.lineWidth = 1;
    ctx.setLineDash([2, 4]);
    ctx.beginPath();
    ctx.moveTo(O.x, O.y);
    ctx.lineTo(D.x, D.y);
    ctx.lineTo(B.x, B.y);
    ctx.lineTo(A.x, A.y);
    ctx.closePath();
    ctx.stroke();
    ctx.setLineDash([]);

    // Base Triangle (Green Glow)
    ctx.save();
    ctx.strokeStyle = "#39ff14";
    ctx.shadowColor = "#39ff14";
    ctx.shadowBlur = 10;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(O.x, O.y);
    ctx.lineTo(A.x, A.y);
    ctx.lineTo(B.x, B.y);
    ctx.stroke();
    ctx.restore();

    // Height (Cyan Glow)
    ctx.save();
    ctx.strokeStyle = "#00e5ff";
    ctx.shadowColor = "#00e5ff";
    ctx.shadowBlur = 10;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(B.x, B.y);
    ctx.lineTo(C.x, C.y);
    ctx.stroke();
    ctx.restore();

    // Space Diagonal (Pink Glow)
    ctx.save();
    ctx.strokeStyle = "#ff0080";
    ctx.shadowColor = "#ff0080";
    ctx.shadowBlur = 12;
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(O.x, O.y);
    ctx.lineTo(C.x, C.y);
    ctx.stroke();
    ctx.restore();

    // Projection Box (Faint)
    ctx.strokeStyle = "rgba(255,255,255,0.1)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    const Az = iso(a, 0, c);
    const Oz = iso(0, 0, c);
    const Dz = iso(0, b, c);
    ctx.moveTo(A.x, A.y); ctx.lineTo(Az.x, Az.y); ctx.lineTo(C.x, C.y);
    ctx.moveTo(Az.x, Az.y); ctx.lineTo(Oz.x, Oz.y); ctx.lineTo(Dz.x, Dz.y); ctx.lineTo(C.x, C.y);
    ctx.moveTo(Oz.x, Oz.y); ctx.lineTo(O.x, O.y);
    ctx.moveTo(Dz.x, Dz.y); ctx.lineTo(D.x, D.y);
    ctx.stroke();

    // Labels
    ctx.fillStyle = "white";
    ctx.font = "bold 14px monospace";
    ctx.fillText(`a=${a}`, (O.x + A.x) / 2, (O.y + A.y) / 2 + 20);
    ctx.fillText(`b=${b}`, (A.x + B.x) / 2 + 10, (A.y + B.y) / 2 + 10);
    ctx.fillText(`c=${c}`, B.x + 10, (B.y + C.y) / 2);

  }, [a, b, c]);

  return (
    <div className="relative w-full h-full bg-[#050505] rounded-xl border border-white/10 shadow-inner overflow-hidden">
      <canvas ref={ref} style={{ width: '100%', height: '100%' }} />
    </div>
  );
}

interface DistanceCanvasProps {
  p1: { x: number; y: number };
  p2: { x: number; y: number };
}

function DistanceCanvas({ p1, p2 }: DistanceCanvasProps) {
  const ref = useRef<HTMLCanvasElement | null>(null);

  useEffect(() => {
    const canvas = ref.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    // HiDPI Scaling
    const dpr = window.devicePixelRatio || 1;
    const size = 600;
    canvas.width = size * dpr;
    canvas.height = size * dpr;
    ctx.scale(dpr, dpr);

    const w = size;
    const h = size;
    ctx.clearRect(0, 0, w, h);

    const pad = 36;
    const range = 12;
    const gridSize = (Math.min(w, h) - pad * 2) / (range * 2);
    const origin = { x: w / 2, y: h / 2 };

    // Grid (Cyber Style)
    ctx.strokeStyle = "rgba(255,255,255,0.05)";
    ctx.lineWidth = 1;
    for (let i = -range; i <= range; i++) {
      const x = origin.x + i * gridSize;
      const y = origin.y + i * gridSize;
      ctx.beginPath(); ctx.moveTo(x, pad); ctx.lineTo(x, h - pad); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(pad, y); ctx.lineTo(w - pad, y); ctx.stroke();
    }

    // Axes
    ctx.save();
    ctx.strokeStyle = "rgba(255,255,255,0.2)";
    ctx.lineWidth = 2;
    ctx.shadowColor = "rgba(255,255,255,0.1)";
    ctx.shadowBlur = 5;
    ctx.beginPath(); ctx.moveTo(pad, origin.y); ctx.lineTo(w - pad, origin.y); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(origin.x, pad); ctx.lineTo(origin.x, h - pad); ctx.stroke();
    ctx.restore();

    const toPx = (p: { x: number; y: number }) => ({
      x: origin.x + p.x * gridSize,
      y: origin.y - p.y * gridSize,
    });

    const P1 = toPx(p1);
    const P2 = toPx(p2);
    const corner = toPx({ x: p2.x, y: p1.y });

    // Triangle Path (Dotted White)
    ctx.setLineDash([4, 4]);
    ctx.strokeStyle = "rgba(255,255,255,0.4)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(P1.x, P1.y);
    ctx.lineTo(corner.x, corner.y);
    ctx.lineTo(P2.x, P2.y);
    ctx.stroke();
    ctx.setLineDash([]);

    // Distance Line (Neon Purple)
    ctx.save();
    ctx.strokeStyle = "#d946ef"; // fuchsia-500
    ctx.shadowColor = "#d946ef";
    ctx.shadowBlur = 12;
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(P1.x, P1.y);
    ctx.lineTo(P2.x, P2.y);
    ctx.stroke();
    ctx.restore();

    // Points (Glowing Dots)
    const dot = (p: { x: number; y: number }, color: string) => {
      ctx.save();
      ctx.fillStyle = color;
      ctx.shadowColor = color;
      ctx.shadowBlur = 10;
      ctx.beginPath();
      ctx.arc(p.x, p.y, 5, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    };

    dot(P1, "#fff");
    dot(P2, "#39ff14"); // Green for target

    // Labels
    ctx.fillStyle = "rgba(255,255,255,0.8)";
    ctx.font = "14px monospace";
    ctx.fillText(`(${p1.x},${p1.y})`, P1.x + 10, P1.y - 10);
    ctx.fillText(`(${p2.x},${p2.y})`, P2.x + 10, P2.y - 10);
  }, [p1, p2]);

  return (
    <div className="relative w-full h-full bg-[#050505] rounded-xl border border-white/10 shadow-inner overflow-hidden">
      <canvas ref={ref} style={{ width: '100%', height: '100%' }} />
    </div>
  );
}

export interface S202CanvasProps {
  visual: {
    kind: "triangle" | "space" | "distance";
    a?: number;
    b?: number;
    c?: number;
    highlightRightAngle?: boolean;
    p1?: { x: number; y: number };
    p2?: { x: number; y: number };
  };
}

export default function S202PythagorasCanvas({ visual }: S202CanvasProps) {
  if (visual.kind === "triangle" && visual.a !== undefined && visual.b !== undefined && visual.c !== undefined) {
    return <TriangleCanvas a={visual.a} b={visual.b} c={visual.c} highlightRightAngle={visual.highlightRightAngle ?? false} />;
  }

  if (visual.kind === "space" && visual.a !== undefined && visual.b !== undefined && visual.c !== undefined) {
    return <SpaceCanvas a={visual.a} b={visual.b} c={visual.c} />;
  }

  if (visual.kind === "distance" && visual.p1 && visual.p2) {
    return <DistanceCanvas p1={visual.p1} p2={visual.p2} />;
  }

  return <div className="text-white/40 text-center p-8">No visualization available</div>;
}
