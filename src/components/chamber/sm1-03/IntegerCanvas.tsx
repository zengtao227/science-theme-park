...
export default function IntegerCanvas({ stage, quest, language }: IntegerCanvasProps) {
  ...
  const visualization = useMemo(() => {
    if (stage === "NUMBER_LINE") {
      // Use thermometer for temperature-related problems, number line for others
      if (quest?.context?.includes("temperature") || quest?.context?.includes("Temperature")) {
        return renderThermometer();
      }
      return renderNumberLine();
    } else if (stage === "RATIONALS") {
      return renderNumberLine();
