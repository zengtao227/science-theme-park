"use client";

import { useCallback, useEffect, useMemo, useRef } from "react";

interface MeasurementCanvasProps {
    tool: string;
    value: number;
    stage: string;
    translations: any;
}

export default function MeasurementCanvas({ tool, value, stage, translations }: MeasurementCanvasProps) {
    const canvasRef = useRef<HTMLCanvasElement>(null);

    // Extract canvas labels from translations
    const canvasLabels = useMemo(() => ({
        ruler: translations("sp3_01.canvas_labels.ruler"),
        ruler_unit: translations("sp3_01.canvas_labels.ruler_unit"),
        scale: translations("sp3_01.canvas_labels.scale"),
        scale_unit: translations("sp3_01.canvas_labels.scale_unit"),
        stopwatch: translations("sp3_01.canvas_labels.stopwatch"),
        stopwatch_unit: translations("sp3_01.canvas_labels.stopwatch_unit")
    }), [translations]);

    const drawRuler = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
        const centerY = h / 2;
        const rulerLength = w * 0.8;
        const startX = (w - rulerLength) / 2;

        ctx.fillStyle = "#1a1a1a";
        ctx.fillRect(startX, centerY - 20, rulerLength, 40);

        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 2;
        ctx.strokeRect(startX, centerY - 20, rulerLength, 40);

        ctx.strokeStyle = "#00ff00";
        ctx.fillStyle = "#00ff00";
        ctx.font = "10px monospace";
        ctx.textAlign = "center";

        for (let i = 0; i <= 10; i++) {
            const x = startX + (rulerLength / 10) * i;
            const tickHeight = i % 5 === 0 ? 15 : 8;

            ctx.beginPath();
            ctx.moveTo(x, centerY - 20);
            ctx.lineTo(x, centerY - 20 + tickHeight);
            ctx.stroke();

            if (i % 5 === 0) {
                ctx.fillText(`${i}`, x, centerY + 5);
            }
        }

        ctx.font = "12px monospace";
        ctx.fillStyle = "#00ff00";
        ctx.textAlign = "center";
        ctx.fillText(`${canvasLabels.ruler} [${canvasLabels.ruler_unit}]`, w / 2, centerY + 35);
    }, [canvasLabels]);

    const drawScale = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, scaleValue: number) => {
        const centerX = w / 2;
        const centerY = h / 2;

        ctx.fillStyle = "#1a1a1a";
        ctx.fillRect(centerX - 80, centerY + 20, 160, 15);

        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 2;
        ctx.strokeRect(centerX - 80, centerY + 20, 160, 15);

        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(centerX, centerY + 35);
        ctx.lineTo(centerX, centerY + 60);
        ctx.stroke();

        ctx.fillStyle = "#0a0a0a";
        ctx.fillRect(centerX - 60, centerY - 40, 120, 40);
        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 2;
        ctx.strokeRect(centerX - 60, centerY - 40, 120, 40);

        ctx.font = "bold 20px monospace";
        ctx.fillStyle = "#00ff00";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(`${scaleValue.toFixed(2)} ${canvasLabels.scale_unit}`, centerX, centerY - 20);

        ctx.font = "12px monospace";
        ctx.fillText(canvasLabels.scale, centerX, centerY + 80);
    }, [canvasLabels]);

    const drawTimer = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, timerValue: number) => {
        const centerX = w / 2;
        const centerY = h / 2;
        const radius = 60;

        ctx.fillStyle = "#0a0a0a";
        ctx.beginPath();
        ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
        ctx.fill();

        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
        ctx.stroke();

        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 2;
        for (let i = 0; i < 12; i++) {
            const angle = (i * 30 - 90) * Math.PI / 180;
            const x1 = centerX + Math.cos(angle) * (radius - 10);
            const y1 = centerY + Math.sin(angle) * (radius - 10);
            const x2 = centerX + Math.cos(angle) * (radius - 5);
            const y2 = centerY + Math.sin(angle) * (radius - 5);

            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
            ctx.stroke();
        }

        const seconds = timerValue % 60;
        const secondAngle = (seconds * 6 - 90) * Math.PI / 180;
        ctx.strokeStyle = "#00ff00";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(centerX, centerY);
        ctx.lineTo(
            centerX + Math.cos(secondAngle) * (radius - 15),
            centerY + Math.sin(secondAngle) * (radius - 15)
        );
        ctx.stroke();

        ctx.fillStyle = "#00ff00";
        ctx.beginPath();
        ctx.arc(centerX, centerY, 4, 0, Math.PI * 2);
        ctx.fill();

        ctx.font = "14px monospace";
        ctx.fillStyle = "#00ff00";
        ctx.textAlign = "center";
        ctx.fillText(`${timerValue.toFixed(1)} ${canvasLabels.stopwatch_unit}`, centerX, centerY + radius + 25);

        ctx.font = "12px monospace";
        ctx.fillText(canvasLabels.stopwatch, centerX, centerY + radius + 45);
    }, [canvasLabels]);

    useEffect(() => {
        const canvas = canvasRef.current;
        if (!canvas) return;

        const ctx = canvas.getContext("2d");
        if (!ctx) return;

        const dpr = window.devicePixelRatio || 1;
        const rect = canvas.getBoundingClientRect();
        canvas.width = rect.width * dpr;
        canvas.height = rect.height * dpr;
        ctx.scale(dpr, dpr);

        const w = rect.width;
        const h = rect.height;

        // Clear
        ctx.fillStyle = "#000";
        ctx.fillRect(0, 0, w, h);

        // Draw based on tool
        if (tool === "ruler") {
            drawRuler(ctx, w, h);
        } else if (tool === "scale") {
            drawScale(ctx, w, h, value);
        } else if (tool === "timer") {
            drawTimer(ctx, w, h, value);
        }

    }, [tool, value, stage, drawRuler, drawScale, drawTimer]);

    return (
        <canvas
            ref={canvasRef}
            className="w-full h-full"
            style={{ width: "100%", height: "100%" }}
        />
    );
}
