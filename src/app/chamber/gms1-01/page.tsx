...
const FractalCanvas = dynamic(() => import("@/components/chamber/gms1-01/FractalCanvas"), { ssr: false });
...
export default function GS1_01_ComplexFractal() {
    ...
    const zoomValue = t("gms1_01.labels.zoom_value").replace("{value}", zoom.toFixed(1));
    const schemeLabels = {
        classic: t("gms1_01.labels.scheme.classic"),
        fire: t("gms1_01.labels.scheme.fire"),
        ice: t("gms1_01.labels.scheme.ice"),
        rainbow: t("gms1_01.labels.scheme.rainbow"),
    };
        ...
        <div className="min-h-screen bg-black text-green-400 font-mono p-4 relative overflow-hidden">
            ...
            <div className="relative z-10 mb-6 border-2 border-cyan-500 p-4 bg-black/80">
                <div className="flex justify-between items-center mb-2">
                    <h1 className="text-2xl font-bold text-cyan-400">{t("gms1_01.title")}</h1>
                    <Link href="/" className="px-4 py-2 border border-cyan-500 hover:bg-cyan-500/20 transition-colors">
                        {t("gms1_01.back")}
                    </Link>
                </div>
                <div className="text-sm text-cyan-300/70">{t("gms1_01.footer_left")}</div>
            </div>
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="border-2 border-green-500 p-4 bg-black/80 space-y-4 overflow-y-auto max-h-[600px]">
                    <div className="border-b border-green-500 pb-2 mb-4">
                        <h2 className="text-lg font-bold text-green-400">{t("gms1_01.monitor_title")}</h2>
                    </div>

                    <div className="space-y-2">
                        <label className="text-sm text-cyan-400">{t("gms1_01.labels.max_iterations")}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm text-purple-400">{t("gms1_01.labels.zoom")}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm text-amber-400">{t("gms1_01.labels.center_x")}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm text-pink-400">{t("gms1_01.labels.center_y")}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm text-green-400">{t("gms1_01.labels.color_scheme")}</label>
                    ...
                    <div className="border border-purple-500 p-3 space-y-2">
                        <div className="text-sm text-purple-400">{t("gms1_01.mandelbrot.title")}</div>
                        <div className="text-xs space-y-1 text-purple-300/80">
                            <div>{t("gms1_01.mandelbrot.line_1")}</div>
                            <div>{t("gms1_01.mandelbrot.line_2")}</div>
                            <div>{t("gms1_01.mandelbrot.line_3")}</div>
                        </div>
                    </div>

                    <div className="border border-cyan-500 p-3 space-y-2">
                        <div className="text-sm text-cyan-400">{t("gms1_01.mission.title")}</div>
                        <div className="text-xs text-cyan-300/80">{t("gms1_01.mission.description")}</div>
                    </div>
                </div>
            </div>
        </div>
