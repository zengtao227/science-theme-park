      ...
      <div className="space-y-8">
        ...
        <div className="text-center space-y-2">
          <h3 className="text-[10px] text-white/60 uppercase tracking-[0.5em] font-black">
            {t('sc1_06.mission.title')}
          </h3>
          <p className="text-base text-white/70 font-mono">
            {t('sc1_06.mission.description')}
          </p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 shadow-sm">
        <h3 className="text-lg font-semibold mb-6 text-center">
          {t('sc1_06.reaction_simulation.title')}
        </h3>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 mb-6">
          <p className="text-blue-800 dark:text-blue-200 text-center">
            {t('sc1_06.reaction_simulation.placeholder_message')}
          </p>
        </div>
        ...
        {quest.energyChange && (
          <div className="text-center mb-6">
            <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${quest.energyChange === 'exothermic'
              ? 'bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-200'
              : 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200'
              }`}>
              {quest.energyChange === 'exothermic'
                ? t('sc1_06.reaction_simulation.exothermic')
                : t('sc1_06.reaction_simulation.endothermic')}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-center">
        <button
          ...
          disabled={completed}
          className={`px-8 py-3 rounded-lg font-semibold transition-colors min-h-[44px] ${completed
            ? 'bg-gray-300 dark:bg-gray-700 text-gray-500 dark:text-gray-500 cursor-not-allowed'
            : 'bg-green-500 hover:bg-green-600 text-white cursor-pointer'
            }`}
        >
          {completed
            ? t('sc1_06.reaction_simulation.completed')
            : t('sc1_06.reaction_simulation.complete')}
        </button>
      </div>
