        ...
        <div className="space-y-8">
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-white/80 font-mono max-w-2xl mx-auto leading-relaxed">
              {stage === "BASIC_TRANSFORMS" && (t?.scenarios?.basic_transforms || "[FALLBACK] Roche Pharmaceutical Molecular Analysis: You are working in Roche Basel's computational chemistry department, using linear transformations to analyze protein molecule symmetry. Each matrix represents a symmetry operation (rotation, reflection, scaling). Identifying transformation types is critical for predicting molecular optical properties.")}
              {stage === "DETERMINANT" && (t?.scenarios?.determinant || "[FALLBACK] Novartis Crystal Structure: You are analyzing drug crystal unit cell structures at Novartis Basel. The determinant represents lattice volume change. det(A)=0 indicates crystal structure collapse, det(A)<0 indicates chirality inversion. Accurate determinant calculation is crucial for predicting drug bioactivity.")}
              {stage === "COMPOSITION" && (t?.scenarios?.composition || "[FALLBACK] University of Basel Robotics: You are programming a robotic arm at Basel University robotics lab. Each joint's motion is represented by a transformation matrix. Composite transformation AB means executing joint A's motion first, then joint B's motion. Matrix multiplication order determines the robot arm's final position.")}
            </p>
          </div>
