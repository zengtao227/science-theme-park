...
const MatrixCanvas = dynamic(() => import("@/components/chamber/gm5-01/MatrixCanvas"), { ssr: false });
...
export default function G5_01_MatrixGeometry() {
    ...
    const detLabel = t("gm5_01.labels.det_value").replace("{value}", det.toFixed(2));
        ...
        <div className="min-h-screen bg-black text-green-400 font-mono p-4 relative overflow-hidden">
            ...
            <div className="relative z-10 mb-6 border-2 border-cyan-500 p-4 bg-black/80">
                <div className="flex justify-between items-center mb-2">
                    <h1 className="text-2xl font-bold text-cyan-400">{t("gm5_01.title")}</h1>
                    <Link href="/" className="px-4 py-2 border border-cyan-500 hover:bg-cyan-500/20 transition-colors">
                        {t("gm5_01.back")}
                    </Link>
                </div>
                <div className="text-sm text-cyan-300/70">{t("gm5_01.footer_left")}</div>
            </div>
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="border-2 border-green-500 p-4 bg-black/80 space-y-4 overflow-y-auto max-h-[600px]">
                    <div className="border-b border-green-500 pb-2 mb-4">
                        <h2 className="text-lg font-bold text-green-400">{t("gm5_01.monitor_title")}</h2>
                    </div>

                    <div className="border border-cyan-500 p-3 space-y-2">
                        <div className="text-sm text-cyan-400">{t("gm5_01.labels.matrix_title")}</div>
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center space-x-2 cursor-pointer">
                            ...
                            <span className="text-yellow-400">{t("gm5_01.labels.show_eigenvectors")}</span>
                        </label>
                        <label className="flex items-center space-x-2 cursor-pointer">
                            <input type="checkbox" checked={showGrid} onChange={(e) => setShowGrid(e.target.checked)} className="w-4 h-4" />
                            <span className="text-purple-400">{t("gm5_01.labels.show_grid")}</span>
                        </label>
                        <label className="flex items-center space-x-2 cursor-pointer">
                            <input type="checkbox" checked={animate} onChange={(e) => setAnimate(e.target.checked)} className="w-4 h-4" />
                            <span className="text-green-400">{t("gm5_01.labels.animate")}</span>
                        </label>
                    </div>

                    <div className="border border-green-500 p-3 space-y-2">
                        <div className="text-sm text-green-400">{t("gm5_01.presets.title")}</div>
                        <div className="grid grid-cols-2 gap-2">
                            <button onClick={() => setMatrix([[2, 0, 0], [0, 2, 0], [0, 0, 1]])}
                                className="px-2 py-1 border border-green-500 hover:bg-green-500/20 text-xs">
                                {t("gm5_01.presets.scale")}
                            </button>
                            <button onClick={() => setMatrix([[0, -1, 0], [1, 0, 0], [0, 0, 1]])}
                                className="px-2 py-1 border border-green-500 hover:bg-green-500/20 text-xs">
                                {t("gm5_01.presets.rotate")}
                            </button>
                            <button onClick={() => setMatrix([[1, 1, 0], [0, 1, 0], [0, 0, 1]])}
                                className="px-2 py-1 border border-green-500 hover:bg-green-500/20 text-xs">
                                {t("gm5_01.presets.shear")}
                            </button>
                            <button onClick={() => setMatrix([[-1, 0, 0], [0, 1, 0], [0, 0, 1]])}
                                className="px-2 py-1 border border-green-500 hover:bg-green-500/20 text-xs">
                                {t("gm5_01.presets.reflect")}
                            </button>
                        </div>
                    </div>

                    <div className="border border-purple-500 p-3 space-y-2">
                        <div className="text-sm text-purple-400">{t("gm5_01.linear.title")}</div>
                        <div className="text-xs space-y-1 text-purple-300/80">
                            <div>{t("gm5_01.linear.line_1")}</div>
                            <div>{t("gm5_01.linear.line_2")}</div>
                            <div>{t("gm5_01.linear.line_3")}</div>
                        </div>
                    </div>

                    <div className="border border-cyan-500 p-3 space-y-2">
                        <div className="text-sm text-cyan-400">{t("gm5_01.mission.title")}</div>
                        <div className="text-xs text-cyan-300/80">{t("gm5_01.mission.description")}</div>
                    </div>
                </div>
            </div>
        </div>
