...
function keyRelationExpression(quest: OlympiadQuest, t: Translator) {
  if (quest.category === "geometry") {
    return `\\text{${t("em3_01.reasons.relation_geometry")}}`;
  }
  if (quest.category === "arithmetic") {
    return `\\text{${t("em3_01.reasons.relation_arithmetic")}}`;
  }
  if (quest.category === "combinatorics") {
    return `\\text{${t("em3_01.reasons.relation_combinatorics")}}`;
  }
  return `\\text{${t("em3_01.reasons.relation_logic")}}`;
}
...
export function solveEM301(quest: OlympiadQuest, t: Translator): Omit<FeedbackContent, "hint"> {
  const steps = [
    ...
    makeStep(3, t("em3_01.reasons.extract_key_relation"), keyRelationExpression(quest, t)),
