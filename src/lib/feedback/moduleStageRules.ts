export const MODULE_STAGE_RULES: Record<string, Record<string, string>> = {
  ...
  "sm1-04": {
    BALANCE: "ax+b=c",
    SOLVE: "x=\\frac{c-b}{a}",
    TRANSFORM: "\\text{Rearrange for any variable}",
    APPLICATIONS: "\\text{Model real situations with equations}",
  },
  "sm2-01": {
    EXPLORE: "\\text{Identify and extend number patterns}",
    ARCHITECT: "a_n=a_1+(n-1)d",
    SCRAPPER: "a_n=a_1\\cdot r^{n-1}",
    SPEEDSTER: "S_n=\\frac{n}{2}(a_1+a_n)",
    VOYAGER: "S_\\infty=\\frac{a_1}{1-r},\\;|r|<1",
    ELITE: "\\text{Mixed sequence challenge}",
  },
  "sm2-02": {
    SOLVE_HYP: "c=\\sqrt{a^2+b^2}",
    SOLVE_LEG: "a=\\sqrt{c^2-b^2}",
    CHECK_RIGHT: "a^2+b^2=c^2",
    MISSION: "\\text{Apply Pythagorean theorem to real problems}",
    MENTAL: "\\text{Mental estimation of square roots}",
    CHAIN: "\\text{Chain of right triangles}",
    DISTANCE: "d=\\sqrt{(x_2-x_1)^2+(y_2-y_1)^2}",
    ELITE_SPACE: "d=\\sqrt{\\Delta x^2+\\Delta y^2+\\Delta z^2}",
  },
  "sm2-08": {
    BASIC_PROB: "P(E)=\\frac{\\text{favorable}}{\\text{total}}",
    LOTTERY: "P(A\\cap B)=P(A)\\cdot P(B)",
    COMBINED: "P(A\\cup B)=P(A)+P(B)-P(A\\cap B)",
    DATA_STATS: "\\bar{x}=\\frac{\\sum x_i}{n},\\;\\sigma^2=\\frac{\\sum(x_i-\\bar{x})^2}{n}",
  },
  "sm2-10": {
    BOX_PLOTS: "\\text{Q1, median, Q3, IQR}",
    SCATTER_PLOTS: "\\text{Plot and interpret bivariate data}",
    CORRELATION: "r=\\frac{\\sum(x_i-\\bar{x})(y_i-\\bar{y})}{(n-1)s_x s_y}",
    ELITE: "\\text{Advanced statistical inference}",
  },
  "sm2-13": {
    reflection: "\\text{Reflect across axis or line}",
    translation: "(x,y)\\to(x+a,\\,y+b)",
    rotation: "(x,y)\\to(x\\cos\\theta-y\\sin\\theta,\\,x\\sin\\theta+y\\cos\\theta)",
    composition: "\\text{Combine two or more transformations}",
  },
  "sm3-01": {
    TERMS: "\\text{Identify and classify polynomial terms}",
    FACTORIZE: "ax^2+bx+c=(px+q)(rx+s)",
    FRACTIONS: "\\frac{p(x)}{q(x)}=\\text{simplify and operate}",
    EQUATIONS: "\\text{Solve polynomial and rational equations}",
  },
};
