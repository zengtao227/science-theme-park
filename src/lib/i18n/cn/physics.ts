...
export const cnPhysics = {
  ...
  gp2_04: {
      ...
      objective_title: "当前任务目标",
      monitor_title: "熵监视器",
      footer_left: "GP2.04_熵与无序 // 节点：巴塞尔",
      check: "验证",
      next: "下一概念",
      correct: "正确",
      incorrect: "错误",
      stages: {
          entropy_concept: "熵的概念",
          second_law: "第二定律",
          arrow_of_time: "时间之箭"
      },
      scenarios: {
          entropy_concept: "巴塞尔大学化学实验室 - 分子无序分析：你在巴塞尔大学物理化学实验室工作，研究熵的概念 - 整个科学中最深刻和最容易被误解的概念之一。熵（S）是无序、随机性或系统可能的微观排列（微观态）数量的度量。在保持相同宏观性质（温度、压强、体积）的同时，分子排列的方式越多，熵就越高。例如，冰的熵很低，因为水分子被锁定在刚性晶体结构中，可能的排列很少。液态水的熵更高，因为分子可以自由移动并占据更多位置。水蒸气（蒸汽）的熵最高，因为气体分子完全自由地在整个容器体积中移动。关系是：固体 < 液体 < 气体（就熵而言）。当冰融化时，熵增加（ΔS > 0），因为系统变得更无序。当水蒸发时，熵增加得更加显著。熵在混合过程中也会增加 - 当你混合热水和冷水，或当你混合两种不同的气体时，最终混合状态的熵高于分离状态，因为分子有更多可能的排列。对于恒温下的可逆过程，熵变为ΔS = Q/T，其中Q是传递的热量，T是绝对温度（开尔文）。这个方程表明，加热增加熵，并且在较低温度下效果更大。理解熵对于预测化学反应是否会自发发生、设计高效的工业过程以及理解为什么某些过程是不可逆的至关重要。你的任务是识别哪些状态具有更高的熵，并理解熵在相变和混合过程中如何变化。",
          second_law: "CERN粒子物理实验室 - 热力学约束：你在日内瓦附近的CERN工作，研究支配宇宙中所有过程的基本物理定律。热力学第二定律是物理学中最基本的原理之一，指出孤立系统的总熵永远不会随时间减少：ΔS_宇宙 ≥ 0。对于可逆（理想）过程，ΔS_宇宙 = 0。对于所有真实（不可逆）过程，ΔS_宇宙 > 0。这个定律具有深远的意义：它定义了时间的方向，解释了为什么某些过程是不可能的，并为能量转换效率设定了基本限制。热量自发地从热物体流向冷物体，因为这增加了总熵 - 冷物体获得的熵多于热物体失去的熵（因为ΔS = Q/T，而T对于冷物体更小）。热量永远不会自发地从冷到热流动，因为那会减少总熵，违反第二定律。这就是为什么冰箱和空调需要功输入 - 你必须做功才能将热量转移到其自然方向的相反方向。第二定律还解释了为什么你不能建造一个完美的热机，将100%的热量转换为功而没有废物 - 必须排出一些热量以增加周围环境的熵。重要的是要理解，熵可以局部减少（就像水在你的冰箱里结冰时），但系统加周围环境的总熵总是增加（冰箱的压缩机产生的熵多于冰失去的熵）。在CERN，第二定律支配着粒子相互作用、加速器中的能量耗散，甚至宇宙向最大熵（热寂）的演化。你的任务是确定过程是否被第二定律允许，并理解为什么某些过程是不可能的。",
          arrow_of_time: "巴塞尔哲学学会 - 时间方向与麦克斯韦妖：你在巴塞尔哲学学会上就物理学最深刻的问题之一进行演讲：为什么时间只向一个方向流动？答案在于熵。'时间之箭'是从过去到未来的单向方向，它由熵增加的方向定义。当你掉落一个玻璃杯并且它破碎时，熵急剧增加 - 有序的结构变成无序的碎片。你永远不会看到破碎的玻璃自发地重新组装，因为那需要熵减少，违反第二定律。当你将牛奶混入咖啡时，分子扩散开来，熵增加。你不能将它们分离，因为混合状态比分离状态具有更多可能的分子排列。所有不可逆过程（摩擦、混合、热流、化学反应）都增加熵并定义时间的前进方向。宇宙作为一个整体是一个孤立系统，因此其总熵不断增加。这导致了'热寂'的概念 - 宇宙的最终命运，当所有能量均匀分布，不存在温度差异，无法做功时。有趣的是，生物体似乎通过从无序中创造秩序（低熵）来违反第二定律 - 一粒种子长成高度有组织的树。然而，生命并不违反热力学，因为生物体不是孤立系统。我们不断消耗能量（食物、阳光）并将熵输出到我们的环境中（热量、废物）。生物体加环境的总熵增加。麦克斯韦妖是一个著名的思想实验：想象一个微小的妖精控制两个气室之间的门，只允许快速分子单向通过，慢速分子另一向通过。这将在没有功的情况下分离热和冷，减少熵并违反第二定律。解决方案是妖精必须测量分子速度，而这个测量过程需要能量并产生熵 - 妖精无法战胜热力学。理解时间之箭将物理学与哲学、生物学和宇宙学联系起来。你的任务是理解为什么过程是不可逆的，熵如何定义时间的方向，以及为什么即使像麦克斯韦妖这样聪明的方案也无法违反第二定律。"
      }
  },
  gp2_05: {
      back: "返回枢纽",
      ...
      scenarios: {
          magnetic_force: "CERN粒子加速器 - 带电粒子的洛伦兹力：你在日内瓦附近的CERN工作，大型强子对撞机（LHC）使用强大的磁场将带电粒子引导在27公里的环形轨道上。当带电粒子在磁场中移动时，它会受到垂直于其速度和磁场方向的力。这被称为洛伦兹力：F = qvB sin(θ)，其中q是粒子的电荷（库仑），v是其速度（m/s），B是磁场强度（特斯拉），θ是速度与磁场之间的角度。当粒子垂直于磁场移动时力最大（θ = 90°，sin(θ) = 1），平行于磁场移动时力为零（θ = 0°，sin(θ) = 0）。力的方向由右手定则给出：将手指指向速度方向，向磁场方向弯曲，拇指指向力的方向（对于正电荷；负电荷相反）。这个垂直力使带电粒子沿圆形路径移动。圆形路径的半径为r = mv/(qB)，其中m是粒子的质量。较重的粒子或较快的粒子具有较大的半径，而较强的磁场或较高的电荷产生更紧的曲线。在CERN，超导磁体产生8.3特斯拉的磁场（是地球磁场的165,000倍），以弯曲以99.9999%光速行进的质子束。LHC使用1,232个偶极磁体保持粒子在轨道上，392个四极磁体聚焦束流。理解带电粒子上的磁力对于粒子物理研究、质谱仪、阴极射线管和聚变反应堆中的等离子体约束至关重要。你的任务是计算磁力，使用右手定则确定力的方向，并理解磁场如何控制带电粒子的运动。",
          magnetic_field: "巴塞尔大学医院 - MRI磁场映射：你与巴塞尔大学医院的医学物理团队合作，校准磁共振成像（MRI）扫描仪。MRI使用极强的磁场（1.5到3特斯拉）来对齐体内的氢核，然后使用无线电波创建内部器官的详细图像。理解磁场强度、方向以及磁场如何产生对于医学成像至关重要。磁场强度B以特斯拉（T）或高斯（G）为单位测量，其中1 T = 10,000 G。地球磁场约为0.00005 T（0.5 G），冰箱磁铁约为0.01 T（100 G），MRI扫描仪使用1.5-3 T（15,000-30,000 G）。磁场由移动电荷（电流）产生。载流I的长直导线在其周围产生圆形磁场，强度为B = (μ₀I)/(2πr)，其中μ₀ = 4π × 10⁻⁷ T·m/A是自由空间的磁导率，r是距导线的距离。磁场方向遵循右手定则：将拇指指向电流方向，手指弯曲的方向就是磁场方向。螺线管（线圈）在内部产生均匀磁场：B = μ₀nI，其中n是单位长度的匝数。这就是电磁铁的工作原理 - 将导线缠绕成许多圈并通过电流产生强大的、可控的磁场。MRI扫描仪使用冷却到-269°C的超导线圈，用液氦携带大电流（数百安培）而没有电阻，产生成像所需的强大均匀磁场。磁场必须在成像体积上极其均匀（变化小于百万分之一）才能获得清晰的图像。理解磁场产生帮助工程师设计MRI扫描仪、电动机、变压器和磁悬浮系统。你的任务是从电流计算磁场强度，理解磁场强度如何随距离变化，并应用右手定则确定磁场方向。",
          applications: "巴塞尔有轨电车系统 - 电磁应用：你与巴塞尔公共交通管理局（BVB）合作，了解该市广泛的有轨电车网络背后的电磁原理。巴塞尔的有轨电车使用电动机，通过电磁力将电能转换为机械运动。当电流在磁场中流过导线时，导线受到力：F = BIL sin(θ)，其中B是磁场强度，I是电流，L是磁场中导线的长度，θ是电流与磁场之间的角度。这是电动机背后的原理 - 磁场中的载流线圈受到使其旋转的扭矩。电动机的旋转线圈称为电枢，固定磁体产生磁场。当线圈旋转时，换向器每半圈反转电流方向，保持连续旋转。现代有轨电车使用带电子控制的三相交流电动机，实现平稳加速和再生制动（减速时将动能转换回电能）。相反的效应也会发生：当导线在磁场中移动时，会在其上感应出电压（电磁感应）。这就是发电机的工作原理 - 机械运动产生电能。巴塞尔的有轨电车使用再生制动在下坡或停车时将电能反馈到架空电力线，将能源效率提高30%。其他电磁应用包括：使用排斥磁力使列车悬浮在轨道上方的磁悬浮列车，速度超过600 km/h；使用磁偏转按质量分离离子的质谱仪；以及像CERN的LHC这样的粒子加速器。霍尔效应是另一个重要现象：当电流流过垂直于电流的磁场中的导体时，导体上会出现电压。这用于霍尔效应传感器测量磁场、检测电动机中的位置以及无接触测量电流。理解电磁应用将基础物理与日常技术联系起来 - 从你乘坐上班的有轨电车，到医院的MRI扫描仪，再到尖端的粒子物理研究。你的任务是理解载流导线上的磁力如何在电动机中产生运动，磁场中移动的导线如何发电，以及这些原理如何实现现代技术。"
      }
  },
  sp3_05: {
      back: "返回枢纽",
      ...
      scenarios: {
          levers: "巴塞尔建筑工地 - 杠杆应用：你在巴塞尔建筑工地工作，工人们使用杠杆来移动重物。杠杆是围绕固定点（称为支点）旋转的刚性杆。杠杆原理指出F₁ × d₁ = F₂ × d₂，其中F是力，d是距支点的距离。这意味着在远离支点处施加的小力可以举起靠近支点的大负载。机械优势（MA）是负载距离与施力距离的比率：MA = d_施力 / d_负载。根据支点、负载和施力的位置，杠杆分为三类。第一类杠杆（如跷跷板和撬棍）支点在施力和负载之间，允许力的放大和方向改变。第二类杠杆（如独轮车和坚果夹）负载在支点和施力之间，总是提供大于1的机械优势。第三类杠杆（如镊子和钓鱼竿）施力在支点和负载之间，提供速度和运动范围而不是力的放大（MA < 1）。在巴塞尔建筑工地，工人使用撬棍（第一类）撬开箱子，独轮车（第二类）运输材料，铲子（第三类）挖掘。理解杠杆帮助工人高效移动重物，同时最小化身体劳损和受伤风险。由于支点处的摩擦和杠杆杆的弯曲，真实杠杆的效率低于100%。你的任务是计算不同杠杆配置的力、距离和机械优势。",
          pulleys: "巴塞尔港口 - 滑轮系统：你在巴塞尔莱茵河港口工作，起重机使用滑轮系统装卸货船。滑轮是带有凹槽边缘的轮子，用于固定绳索或缆绳。滑轮改变力的方向并可以提供机械优势。定滑轮固定在静止物体上，只改变力的方向 - 你向下拉以向上提升，但所需的力等于负载重量（MA = 1）。动滑轮连接到负载并随之移动。负载由两段绳索支撑，因此施力是负载重量的一半（MA = 2），但你必须拉两倍的距离。复合滑轮系统结合定滑轮和动滑轮以实现更高的机械优势。滑轮系统的MA等于支撑负载的绳索段数。例如，具有4根支撑绳索的系统MA = 4，意味着你只需要1/4的力来提升负载，但必须拉4倍的距离。这展示了功的原理：在理想机器中，输入功（施力 × 施力距离）等于输出功（负载力 × 负载距离）。由于绳索摩擦和滑轮轮摩擦，真实滑轮系统的效率低于100%。在巴塞尔港口，起重机使用MA = 6到10的复合滑轮系统来提升重达数吨的集装箱。理解滑轮帮助工程师设计高效的提升系统，帮助工人安全操作它们。你的任务是计算不同滑轮配置的力、距离、机械优势和功。",
          inclined_planes: "巴塞尔山地铁路 - 斜面力学：你正在分析巴塞尔的Gellert山缆车，它使用斜面原理将乘客运送到陡坡上。斜面（坡道）是与水平面成一定角度倾斜的平面。它允许用比垂直提升更小的力来提升重物，代价是在更长的距离上移动它们。斜面的机械优势是MA = L / h，其中L是坡道的长度，h是垂直高度。例如，一个5米长上升1米的坡道MA = 5，意味着你只需要1/5的力将物体推上坡道，而不是垂直提升它。然而，你必须推5倍的距离。坡道越陡（角度越大），给定高度的长度越短，导致MA越低，需要更多的力。斜面在巴塞尔随处可见：巴塞尔SBB火车站的轮椅坡道（缓坡，高MA），汝拉山的山路（之字形减少陡度）和仓库的装载坡道。两种相关的简单机械源自斜面：楔子（如斧头和刀）是移动的斜面，用于分开物体，螺丝是缠绕在圆柱体上的斜面，将旋转运动转换为线性运动。斜面的效率取决于摩擦 - 光滑表面（冰、滚轮）比粗糙表面效率更高。你的任务是计算机械优势、力，并理解斜面如何减少提升物体所需的力，同时增加行进的距离。"
      }
  },
  sp3_06: {
      back: "返回枢纽",
      ...
      stages: {
          sound_waves: "声波",
          frequency_pitch: "频率与音调",
          loudness_intensity: "响度与强度"
      },
      scenarios: {
          sound_waves: "巴塞尔交响乐团 - 声波物理：你正在与巴塞尔交响乐团在Stadtcasino Basel（欧洲最好的音乐厅之一）合作，了解声音如何从乐器传播到观众。声音是一种机械波，需要介质（固体、液体或气体）才能传播 - 它不能在真空中传播。当小提琴弦振动时，它推拉空气分子，产生压缩区（高压）和稀疏区（低压），这些区域作为纵波向外传播。声速取决于介质：在20°C的空气中约为343 m/s，在水中约为1,480 m/s，在钢中约为5,960 m/s。声音在密度更大的介质中传播更快，因为分子更靠近，可以更快地传递振动。波长λ、频率f和速度v的关系为v = f × λ。例如，空气中440 Hz的音符（A4）的波长λ = 343/440 ≈ 0.78 m。声波可以反射（回声）、折射（进入不同介质时弯曲）、衍射（绕过障碍物弯曲）和干涉（建设性或破坏性组合）。Stadtcasino的声学设计使用反射均匀分布声音 - 弯曲的墙壁和天花板面板将声波引导到所有座位。理解声波物理帮助建筑师设计音乐厅，工程师开发降噪系统，音乐家优化他们的乐器。你的任务是计算不同介质和情况下的波长、频率和声速。",
          frequency_pitch: "巴塞尔音乐学院 - 频率与音调：你在巴塞尔音乐学院（Musik-Akademie Basel）学习，了解频率与音乐音调之间的关系。频率是每秒振动（周期）的次数，以赫兹（Hz）为单位测量。人类听觉范围约为20 Hz（非常低的低音）到20,000 Hz（非常高的高音），尽管这个范围随年龄而减少。音调是我们对频率的感知 - 频率越高，音调越高。音乐音符有特定的频率：中央C（C4）是261.6 Hz，A4（音乐会音高）是440 Hz，C5是523.2 Hz。音符之间的关系遵循对数刻度 - 每个八度音程使频率加倍。例如，A5（880 Hz）比A4（440 Hz）高一个八度。相邻钢琴键（半音）之间的间隔将频率乘以2^(1/12) ≈ 1.0595。乐器产生具有基频（最低）和谐波（整数倍）的复杂声音。演奏A4的小提琴产生440 Hz（基频）、880 Hz（第二谐波）、1320 Hz（第三谐波）等。谐波的混合赋予每种乐器独特的音色（音质）。超声波（>20 kHz）用于医学成像和动物通信 - 蝙蝠使用20-120 kHz的回声定位。次声波（<20 Hz）由地震、火山和大型动物如大象产生。在巴塞尔音乐学院，学生学习通过匹配频率来调音乐器，并通过频率比识别音程。理解频率帮助音乐家调音乐器，音频工程师设计音响系统，医生使用超声波进行医学诊断。",
          loudness_intensity: "巴塞尔机场 - 声强与响度：你在EuroAirport Basel-Mulhouse-Freiburg工作，测量和控制噪音水平，以确保乘客舒适和符合法规。声强是声波携带的单位面积功率，以瓦特每平方米（W/m²）为单位测量。响度是我们对强度的感知，以分贝（dB）为单位测量。分贝刻度是对数的：L = 10 log₁₀(I/I₀)，其中I₀ = 10⁻¹² W/m²是听觉阈值。耳语约为30 dB，正常对话60 dB，吸尘器70 dB，摇滚音乐会110 dB，喷气发动机140 dB（疼痛阈值）。因为刻度是对数的，10 dB的增加代表强度增加10倍 - 70 dB比60 dB强10倍，而不仅仅是10%。长时间暴露于85 dB以上的声音会导致听力损伤。声强随距离按平方反比定律减少：如果你将距声源的距离加倍，强度减少到1/4（响度减少6 dB）。在巴塞尔机场，隔音屏障和发动机定位减少了到达附近社区的声音。飞机起飞在100米处产生约130 dB，但由于距离和空气吸收，在1公里处只有100 dB。理解声强帮助工程师设计更安静的机器，建筑师规划降噪，监管机构制定安全标准。你的任务是计算声强，在强度和分贝之间转换，并理解距离如何影响响度。"
      }
  },
};
