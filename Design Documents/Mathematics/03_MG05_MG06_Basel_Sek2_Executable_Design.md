...
### 3.2 推荐 Stage 结构（与 S3.01 一致：阶段化，而非纯模式列表）
...
- 文案不使用扁平 key 堆叠；对提示按题型与层级组织为结构化对象，例如：
  - `s3_01.hints.factorize.trinomial.layer1`
  - `s3_01.hints.factorize.trinomial.layer2`
  - `s3_01.hints.fractions.cancel.layer1`
...
1) 先稳定 S3.01：把 Stage 题库补齐、i18n 文案完善、提示层级一致化。
2) 完成 Basel-Stadt + Baselland 的 S2.02 进度对齐总结（写入设计稿附录）。
3) 再改 S2.02：按 Stage 逐个替换/实现（优先坐标距离可视化 + 根式输入）。
